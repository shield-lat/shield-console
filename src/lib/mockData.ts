...
export const mockAttackEvents: AttackEvent[] = [
  ...
  },
  {
    ...
    createdAt: hoursAgo(12),
    details:
      "Attempted bulk export of transaction history beyond authorized scope",
  },
  {
    id: randomId(),
    applicationId: "app-mobile-banking",
...
function generateTimeSeriesData(days: number): TimeSeriesDataPoint[] {
  ...
  for (let i = days * 24; i >= 0; i -= 4) {
    ...
    data.push({
      ...
      allow: Math.floor(baseVolume * (0.85 + Math.random() * 0.1)),
      requireHitl: Math.floor(baseVolume * (0.08 + Math.random() * 0.04)),
      block: Math.floor(baseVolume * (0.02 + Math.random() * 0.02)),
    });
  }
  return data;
}
